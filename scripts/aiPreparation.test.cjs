...
(async () => {
  ...
  assert.deepEqual((await conversationService.loadConversation()).map(({ text }) => text), ['Test numéro 1', 'Deuxième message']);
  ...
  const talkScreen = fs.readFileSync('./src/screens/TalkScreen.js', 'utf8'); assert.ok(talkScreen.includes('aucune donnée envoyée à une IA')); assert.ok(talkScreen.includes('onManageMemory')); assert.ok(talkScreen.includes('Continue à enregistrer quelques situations')); assert.ok(talkScreen.includes('Effacer la conversation')); assert.ok(talkScreen.includes("text: 'Annuler'")); assert.ok(talkScreen.includes('loadConversation')); assert.ok(talkScreen.includes('addUserMessage')); assert.ok(talkScreen.includes('KeyboardAvoidingView')); assert.ok(talkScreen.includes('<FlatList')); assert.ok(talkScreen.includes('scrollToLatest')); assert.ok(talkScreen.includes('Saisie vocale, bientôt disponible')); assert.ok(talkScreen.includes('contextOpen')); assert.equal(talkScreen.includes('<ScrollView'), false);
