...
export default function EntryFormScreen({ initialEventType, editingEvent, onBack, onSave }) {
  ...
  const [showSubstanceForm,setShowSubstanceForm]=useState(!(editingEvent?.substances||[]).length);
  ...
  const changeEventType=(type)=>{ if(type===eventType)return; setEventType(type);setSelectedSubstances([]);setShowSubstanceForm(true);setPendingSubstance('');setCustomSubstance('');setQuantity('');setUnit('');setStrategies([]); };
  ...
  const addSubstance=()=>{
    ...
    setShowSubstanceForm(false);
  };
  const removeSubstance=(name)=>setSelectedSubstances(current=>{
    const remaining=current.filter(value=>value.name!==name);
    if(!remaining.length)setShowSubstanceForm(true);
    return remaining;
  });
  const submit=()=>{
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
    ...
    </>:<Pressable onPress={()=>setShowSubstanceForm(true)} style={styles.addAnother}><Text style={styles.addText}>+ Ajouter une autre substance</Text></Pressable>}
